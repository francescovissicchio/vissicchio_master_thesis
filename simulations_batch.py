...
# Parametri richiesti da ogni modello
param_prompts = {
    "SIS": ["beta", "mu"],
...
# Simulazioni con barra di progresso
for i in tqdm(range(1, 201), desc="Simulazioni"):
    try:
        ...
        for is_agg in [True, False]:
            suffix = "agg" if is_agg else "np"
            filename = os.path.join(sim_folder, f"{base_name}_{suffix}.csv")
            sim.save_to_csv(filename=filename, aggregate=is_agg)
