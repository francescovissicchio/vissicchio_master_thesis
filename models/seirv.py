...
def state_transition(G, current_state, beta, alpha, gamma, vacc_prob):
    next_state = {}
...
def get_simulation(G, params):
    ...
    def transition(G, state):
        return state_transition(G, state, beta, alpha, gamma, vacc_prob)

    return Simulation(G, initial_state=initial_state, state_transition=transition, name="SEIRV")
