...
VACC_PROB = 0.01 # vaccinazione
QUAR_PROB = 0.1  # quarantena

def initial_state_seirvq(G):
    state = {node: 'S' for node in G.nodes}
    patient_zero = rd.choice(list(G.nodes))
    state[patient_zero] = 'E'
    return state

def state_transition_seirvq(G, current_state):
    next_state = {}
    ...
    for node in G.nodes:
        ...
        # R e V rimangono tali

    return next_state

class SimulationSEIRVQ:
    def __init__(self, G, initial_state=initial_state_seirvq, state_transition=state_transition_seirvq,
                 stop_condition=None, name='SEIRVQ Simulation'):
        self.G = G.copy()
